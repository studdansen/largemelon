...
namespace largemelon {
	 ...
	 * 
	 * @note A newline sequence is <tt>"\r\n"</tt> on Windows, <tt>"\r"</tt> on
	 *   macOS, and <tt>"\n"</tt> on Unix-like OSes.
	 * 		
	 ...
	 * @todo Performance might be boosted by reimplementing without regexes.
	 * 
	 * @code{.cpp}
 ...
 * @param CONTEXT_TYPE Type for the <tt>%extra_argument</tt> directive in the
 *   Lemon parser specification.
 * @details When Lemon generates a parser, it instantiates the
 ...
 * @warning This macro must be called from outside a C++ namespace.*/
...
/**@def LARGEMELON_IS_NEXT_EXP_TOKEN_ID_OLDER
 * @brief Macro resolving to a function call that returns whether a given token
 *   ID is currently a next-expected token ID in a given parser instance. Use
 *   this if Lemon is generating the older token interface.
 * @param PPARSER Instance of parser (i.e., @c yypParser). Cannot be a null
 *   pointer.
 ...
 * @brief Macro resolving to a function call that returns whether a given token
 *   ID is currently a next-expected token ID in a given parser instance. Use
 *   this if Lemon is generating the newer token interface.
 * @param PPARSER Instance of parser (i.e. @c yypParser). Cannot be a null
 *   pointer.
 * @param TOKEN_ID Numeric identifier of queried token.
